...
const testComputers = setupComputersTest({
  ...
  //   return (i) => listener(h(i))
  // },
  async statx({listener, startCreation, endCreation}) {
    const {state, computed} = await import('@statx/core')
  ...
  async 'effector.fork'({listener, startCreation, endCreation}) {
    const {createEvent, createStore, combine, allSettled, fork} = await import('effector')
  ...
  async wonka({listener, startCreation, endCreation}) {
    const {makeSubject, pipe, map, subscribe, combine, sample} = await import('wonka')

    const ccombine = <A, B>(sourceA: WSource<A>, sourceB: WSource<B>): WSource<[A, B]> => {
...
function setupComputersTest(tests: Rec<Setup>) {
  return async (iterations: number, creationTries: number) => {
    ...
    if (creationTries > 0) {
      console.log(`Median of computers creation and linking from ${creationTries} iterations\n(UNSTABLE)`)

      printLogs(Object.fromEntries(testsList.map(({name, creationLogs}) => [name, formatLog(creationLogs)])))
    }
    ...
    while (i++ < iterations) {
      ...
      for (const test of testsList) {
        globalThis.gc?.()
        globalThis.gc?.()
        const mem = globalThis.process?.memoryUsage?.().heapUsed
    ...
    if (globalThis.gc) {
      ...
      printLogs(
        testsList.reduce((acc, {name, memLogs}) => ((acc[name] = formatLog(memLogs)), acc), {} as Rec<any>),
      )
    }
